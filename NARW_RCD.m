...
for i = 1:size(T,1)
    ...
    d_RCD(i,1:2) = [circ_mean(myDiff(st:ed)) circ_std(myDiff(st:ed))]; % mean/STD RCD in dive
    figure(3), hold on, rose(myDiff(st:ed))
end
for i = 1:size(T,1)-1
    st = nearest(t_rs',T(i,2)); % end of one dive
    ed = nearest(t_rs',T(i+1,1)); % start of next dive
    s_RCD(i,1:2) = [circ_mean(myDiff(st:ed)) circ_std(myDiff(st:ed))]; % mean/STD RCD at surface
...
% right now s_RCD and d_RCD are mean and SD RCD per dive 
