...
if exist('dive','var') == 1
    for j = 1:size(T,1)
        ...
        if isempty(dive(j).stops) ~= 1
            ...
            %h = errorbar(T(j,1),mean(dive(j).dens)*1000000,std(dive(j).dens*1000000),'go','markerfacecolor','g'); % in g/m^3
            %plot(dive(j).stops(:,1),dive(j).dens*1000000,'.-','color',[0.7 1 0.7]) % in g/m^3
