...
% 9 is dives to analyze (i.e. d2after)
%%
...
for i = 1:size(tags,1)
    ...
    subplot('position',[0.13 0.98-(tags{i,3}*0.09) 0.86 0.08]), hold on, box on
    UTC = tags{i,2}(4:end);
    ...
    if i == 10, d2after = 1:7; d2after = d2after'; end
    ...
    for k = tags{i,9}' % all dives in analysis
        ...
        if isempty(dive(k).vperblock) == 1
            ...
            phase_pitch(k,1) = nanmean(rad2deg(pitch(round(T(k,1):T(k,4)*fs))));
            % ascent speed
            phase_speed(k,2) = nanmean(dive(k).flowEst(round((T(k,4):T(k,2)-10)-T(k,1))));
            phase_pitch(k,2) = nanmean(rad2deg(pitch(round((T(k,4):T(k,2))*fs))));
        end
...
figure(100),
%[ax1,ha] = suplabel('Local Time','x');
[ax1,ha] = suplabel('Depth (m)','y');
